...
function draw(arr,cxt) {
	cxt.save();
context.clearRect(0,0,302,632);
context.fillStyle = "#EFEFEF";
context.fillRect(0,0,302,632);
cxt.restore();
for(var i=0; i<21; i++) {
	for(var j=0; j<10; j++) {
		if(arr[i][j] !== 0) {
			drawRect(getPosX(i,j),getPosY(i,j),30,30);
		}
	}
}
}
...
//本函数是产生随机形状的方块
//产生7种不同的形状
function randRect(arr) {
	
	shop =1;//每次产生新的方块时，进行初始化
	rand = ~~(Math.random()*7);//产生0-6的随机数
	switch(rand) {
		case 0:
		randNum = 0;
		arr[0][3] = 2;
